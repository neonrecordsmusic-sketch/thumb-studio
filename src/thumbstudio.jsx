...
export default function ThumbStudio() {
  ...
  useEffect(() => {
    ...
    const g = localStorage.getItem("ts2_groq_key");
  ...
  function saveApiKey() {
    ...
    localStorage.setItem("ts2_groq_key", groqKey);
  ...
  async function generateBlueprint() {
    ...
    const systemPrompt =
      lang === "pt"
        ? "Você é um diretor de arte especialista em thumbnails virais do YouTube. Crie blueprints ultra-detalhados e cinematográficos com descrições visuais ricas, paleta de cores precisa e prompts de imagem prontos para geração por IA. Responda em português."
        : lang === "es"
        ? "Eres un director de arte experto en miniaturas virales de YouTube. Crea blueprints ultra-detallados y cinematográficos con descripciones visuales ricas, paleta de colores precisa y prompts de imagen listos para generación por IA. Responde en español."
        : "You are an art director specialized in viral YouTube thumbnails. Create ultra-detailed, cinematic blueprints with rich visual descriptions, precise color palettes and image prompts ready for AI generation. Respond in English.";

    const userMsg = `Create a ULTRA-DETAILED thumbnail BLUEPRINT in the style of a professional art director for:

⚠️ MOST IMPORTANT ELEMENT — READ THIS FIRST:
**🎬 VIDEO TITLE: "${videoTitle || niche}"**
This title is the SOUL of the thumbnail. Every visual decision — composition, colors, typography, character expression, background, effects — must serve and amplify this title. The thumbnail must make the viewer FEEL the title before they even read it. Do NOT ignore this. Build everything around it.

**Niche / Topic:** ${niche}
**Format:** ${dimInfo?.label} (${dimInfo?.sub})
**Visual Style:** ${visualStyle}
${hasCharacter ? `**Character:** Gender: ${gender} | Life Stage: ${lifeStage} | Skin Tone: ${skinTone} | Expression: ${expression}` : "**Character:** None"}
**Text Layers:**
${layerInstructions}
...
**🎨 Visual Composition**
[Describe the full scene in cinematic detail: character pose, clothing, accessories, facial expression, background elements, lighting setup (rim light, key light, fill), atmosphere, depth of field, camera angle]

**🎵 Niche-Specific Visuals**
[Describe 3-5 unique visual elements specific to this niche: floating objects, neon signs, textures, props, environmental details that make this thumbnail instantly recognizable to the target audience]

**Text Layout & Typography Architecture**
[For EACH text layer, describe exactly:
- Layer name and text content
- Position and orientation
- Font style (bold/italic/distressed/3D/chrome/neon)
- Color with hex if possible
- Shadow, glow or outline effects
- Size relative to frame]

**Concept:** [One line describing the overall concept]
**Color Palette:** [List 4-5 specific colors with names]
**Feeling:** [List 4-5 emotional adjectives]

**Prompt for Image Generation:**
> [Write a single, detailed, comma-separated image generation prompt in English. Include: subject description, clothing, expression, background, lighting, text overlays with position, style keywords, aspect ratio --ar ${dimInfo?.sub || "16:9"}]`;


    try {
      const groqKey = localStorage.getItem("ts2_groq_key") || "";
      if (!groqKey) { setBlueprint("⚠️ Configure sua Groq API Key em API SETTINGS."); setIsLoading(false); return; }
      const fullPrompt = systemPrompt + "\n\n" + userMsg;
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${groqKey}` },
        body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages: [{ role: "user", content: fullPrompt }], max_tokens: 2000 })
      });
      ...
      <div style={S.main}>
        ...
        <div style={S.sidebar}>
          <div style={S.sideScroll}>
            ...
            <SideSection label="🎬 TÍTULO DO VÍDEO">
              <textarea className="ts-input" rows={2}
                placeholder="ex: Eu Ganhei R$10k em 30 Dias Fazendo Isso..."
                value={videoTitle} onChange={e => setVideoTitle(e.target.value)} />
              <div style={{ fontSize:10, color:"#ff0000", marginTop:6, fontWeight:700, letterSpacing:0.5 }}>
                ⚡ ELEMENTO MAIS IMPORTANTE — define toda a composição visual
              </div>
            </SideSection>
            ...
            <div style={S.cardHeader}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <span style={S.cardTitle}>
                  {activeTab==="preview" ? t.livePreview : t.blueprintResult}
                </span>
        ...
        <div style={S.rightSidebar}>
          ...
          <div style={{ padding:"12px 14px", margin:"8px", background:"#141414", border:"1px solid #2a2a2a", borderRadius:8 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, color: apiKey?"#22aa44":"#cc4400" }}>
              ...
              {apiKey ? (apiProvider==="gemini" ? t.geminiConnected : "OpenAI Connected") : t.noKey}
            </div>
        ...
        <div style={S.overlay} onClick={() => setShowApiModal(false)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:"flex", gap:10, marginBottom:20 }}>
              {[
                { id:"gemini", emoji:"🔵", label:"Google Gemini", sub:"Imagens (AIza...)" },
                { id:"openai", emoji:"🤗", label:"HuggingFace",   sub:"FLUX.1 — Grátis (hf_...)" },
              ].map(p => (
            ...
            <div style={{ padding:"12px 14px", background:"#141414", border:"1px solid #2a2a2a", borderRadius:8, marginBottom:16, fontSize:12, color:"#777", lineHeight:1.8 }}>
              {apiProvider==="gemini" ? t.geminiInstructions : <>Cadastre-se em <span style={{color:"#ff0000",fontWeight:700}}>huggingface.co</span> → Settings → Access Tokens → New Token (Read). Grátis.</>}
            </div>

            <div style={{ marginBottom:12 }}>
              <div style={{ fontSize:11, color:"#555", fontWeight:700, marginBottom:8, letterSpacing:1 }}>{t.apiKeyLabel} — IMAGENS</div>
            ...
            <div style={{ marginBottom:16 }}>
              <div style={{ fontSize:11, color:"#22aa44", fontWeight:700, marginBottom:8, letterSpacing:1 }}>GROQ API KEY — BLUEPRINT (GRATUITO)</div>
              <input className="ts-input" type="password"
                placeholder="gsk_..."
                value={groqKey} onChange={e => setGroqKey(e.target.value)} />
              <div style={{ fontSize:10, color:"#555", marginTop:6 }}>Obtenha grátis em <span style={{color:"#ff0000"}}>console.groq.com</span> → API Keys</div>
            </div>
