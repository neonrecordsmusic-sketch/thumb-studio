...
End with: BLUEPRINT V4.1 FINAL | ${dimInfo?.sub} FORMAT`;
    ...
    try {
      const openaiKey = localStorage.getItem("ts2_api_key") || "";
      if (!openaiKey) { setBlueprint("⚠️ Configure sua OpenAI API Key em API SETTINGS."); setIsLoading(false); return; }
      const fullPrompt = systemPrompt + "\n\n" + userMsg;
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${openaiKey}` },
        body: JSON.stringify({ model: "gpt-3.5-turbo", messages: [{ role: "user", content: fullPrompt }], max_tokens: 1000 })
      });
